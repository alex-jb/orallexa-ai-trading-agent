...
export function DecisionCard({ d, asset, strategy, horizon, news, risk, investmentPlan, t, zh }: {
  ...
  return (
    <div className="relative" style={{ background: "linear-gradient(180deg, #1A1A2E 0%, #0D1117 100%)" }}>
      ...
      <div className="px-8 pb-4 text-[7px] font-[Lato] text-[#4A4D55] italic leading-relaxed">
        {zh
          ? "⚠️ 本分析由 AI 模型生成（9 ML 模型 + Claude 对抗辩论），仅供研究参考，不构成投资建议。市场有风险，投资需谨慎。"
          : "⚠️ Generated by AI (9 ML models + adversarial debate). For research only — not financial advice. Trade at your own risk."}
      </div>
    </div>
  );
}
