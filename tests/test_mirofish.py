...
class TestScenarioSim:
    ...
    @patch("engine.scenario_sim._build_current_context", return_value="NVDA: $900.00 (+1.2%)")
    ...
    def test_run_scenario_happy_path(self, mock_logged, mock_client, mock_ctx):
        ...
        payload = {
            "impacts": [
                {"ticker": "NVDA", "impact_pct": -5.0, "direction": "bearish",
                 "severity": "high", "reasoning": "Rate hike hurts growth.",
                 "time_horizon": "1-2 weeks", "key_level": 850.0},
            ],
            "portfolio_delta_pct": -5.0,
            "historical_analog": {"event": "Dec 2018", "date": "2018-12",
                                  "market_reaction": "fell", "relevance": "high"},
            "hedging_suggestions": ["Buy puts"],
            "summary": "Bearish scenario.",
            "confidence": 72,
            "regime_shift": "risk-off",
        }
        mock_logged.return_value = (self._mock_llm_response(payload), {})
    ...
    @patch("engine.scenario_sim.get_client")
    @patch("engine.scenario_sim.logged_create")
    def test_run_scenario_normalizes_direction(self, mock_logged, mock_client, mock_ctx):
        from engine.scenario_sim import run_scenario

        payload = {
            "impacts": [
                {"ticker": "AAPL", "impact_pct": 2.0, "direction": "BULLISH",
                 "severity": "WRONG", "reasoning": "ok",
                 "time_horizon": "1w", "key_level": 200},
            ],
            "portfolio_delta_pct": 2.0, "summary": "ok", "confidence": 55,
        }
        mock_logged.return_value = (self._mock_llm_response(payload), {})

        result = run_scenario("good news", ["AAPL"])
        # direction normalised to lowercase; "bullish" is valid
        assert result["impacts"][0]["direction"] == "bullish"
        # invalid severity -> medium
        assert result["impacts"][0]["severity"] == "medium"

    @patch("engine.scenario_sim._build_current_context", return_value="")
    @patch("engine.scenario_sim.get_client")
    @patch("engine.scenario_sim.logged_create", side_effect=Exception("LLM down"))
    def test_run_scenario_fallback_on_error(self, mock_logged, mock_client, mock_ctx):
        from engine.scenario_sim import run_scenario
    ...
    @patch("engine.scenario_sim.get_client")
    @patch("engine.scenario_sim.logged_create")
    def test_run_scenario_equal_weights_default(self, mock_logged, mock_client, mock_ctx):
        from engine.scenario_sim import run_scenario

        payload = {"impacts": [], "portfolio_delta_pct": 0, "summary": "", "confidence": 50}
        mock_logged.return_value = (self._mock_llm_response(payload), {})

        result = run_scenario("neutral", ["A", "B", "C", "D"])
        # should not crash; equal weights internally
        assert result["confidence"] == 50

    @patch("engine.scenario_sim._build_current_context", return_value="")
    @patch("engine.scenario_sim.get_client")
    @patch("engine.scenario_sim.logged_create")
    def test_confidence_clamped(self, mock_logged, mock_client, mock_ctx):
        from engine.scenario_sim import run_scenario

        payload = {"impacts": [], "portfolio_delta_pct": 0, "summary": "", "confidence": 999}
        mock_logged.return_value = (self._mock_llm_response(payload), {})
...
class TestBiasTracker:
    ...
    def test_detect_directional_bias(self):
        ...
        patterns = _detect_patterns(
            ...
            buy_acc=0.8, sell_acc=0.4,
            calibration=[], ticker_stats={},
        )
    ...
    def test_detect_overconfidence(self):
        ...
        calibration = [
            {"range": "0-35", "label": "very_low", "count": 10, "accuracy": 0.7, "avg_return": 0.01},
            {"range": "35-50", "label": "low", "count": 5, "accuracy": 0.6, "avg_return": 0.01},
            {"range": "50-65", "label": "moderate", "count": 5, "accuracy": 0.5, "avg_return": 0.0},
            {"range": "65-100", "label": "high", "count": 10, "accuracy": 0.45, "avg_return": -0.01},
        ]
        evaluated = [{"decision": "BUY"}] * 10
    ...
    def test_detect_ticker_weakness(self):
        ...
        ticker_stats = {"TSLA": {"count": 5, "accuracy": 0.2, "avg_return": -0.03}}
        evaluated = [{"decision": "BUY"}] * 5
    ...
    def test_build_recommendations_directional(self):
        ...
        patterns = [{"type": "directional_bias", "bias_direction": "SELL", "magnitude": 0.2}]
    ...
    def test_evaluate_decisions_basic(self):
        from engine.bias_tracker import _evaluate_decisions

        decisions = [
            {"ticker": "NVDA", "decision": "BUY", "timestamp": datetime.now().isoformat(),
             "confidence": 70, "risk_level": "LOW", "mode": "scalp", "source": "test"},
        ]
        with patch("engine.bias_tracker._get_forward_returns_batch",
    ...
    def test_evaluate_decisions_sell_correct(self):
        ...
        decisions = [
            {"ticker": "AAPL", "decision": "SELL", "timestamp": ts,
             "confidence": 60, "risk_level": "HIGH", "mode": "predict", "source": "test"},
        ]
        with patch("engine.bias_tracker._get_forward_returns_batch",
...
class TestRoleMemory:
    ...
    def test_record_prediction(self, tmp_path):
        ...
        mem.record_prediction("Conservative Analyst", "NVDA", "BULLISH", 40, 70,
                              reasoning="Strong earnings.", key_factor="Earnings beat")
    ...
    def test_update_outcomes_wrong(self, tmp_path):
        mem = self._make_memory(tmp_path)
        mem.record_prediction("Conservative Analyst", "GLD", "BULLISH", 30, 65)
    ...
    def test_get_role_context_with_data(self, tmp_path):
        mem = self._make_memory(tmp_path)
        for i in range(5):
            mem.record_prediction("Conservative Analyst", "NVDA", "BULLISH", 30, 60)
            mem.update_outcomes("NVDA", actual_return=0.02)
...
class TestSignalFusion:
    ...
    def test_score_technical_overbought(self):
        ...
        result = _score_technical({"rsi": 75, "close": 100, "ma20": 95, "ma50": 90})
        assert result["score"] < 0  # overbought RSI
    ...
    def test_score_technical_bullish_ma(self):
        ...
        result = _score_technical({"rsi": 55, "close": 110, "ma20": 105, "ma50": 100})
        assert "Bullish MA alignment" in result["signals"]

    def test_score_technical_bearish_ma(self):
        from engine.signal_fusion import _score_technical

        result = _score_technical({"rsi": 45, "close": 90, "ma20": 95, "ma50": 100})
    ...
    def test_score_ml_with_models(self):
        ...
        ml_result = {
            "results": {
                "random_forest": {"status": "ok", "metrics": {"sharpe": 1.5, "total_return": 0.1}},
                "xgboost": {"status": "ok", "metrics": {"sharpe": 0.8, "total_return": 0.05}},
            }
        }
        result = _score_ml(ml_result)
        assert result["available"] is True
        assert result["n_models"] == 2
        assert result["score"] > 0  # positive sharpe
    ...
    def test_fuse_signals_all_sources(self, mock_inst, mock_opt):
        ...
        summary = {"rsi": 30}
        ml_result = {"results": {"random_forest": {"status": "ok", "metrics": {"sharpe": 2.0, "total_return": 0.15}}}}
        news = [{"score": 0.4}, {"score": 0.3}]

        result = fuse_signals("NVDA", summary=summary, ml_result=ml_result, news_items=news)
    ...
    def test_fuse_signals_custom_weights(self, mock_inst, mock_opt):
        ...
        custom = {"technical": 1.0, "ml_ensemble": 0, "news_sentiment": 0,
                  "options_flow": 0, "institutional": 0}
...
class TestMicroSwarm:
    ...
    def test_swarm_agent_momentum_positive_shock(self):
        ...
        agent = SwarmAgent(name="M1", agent_type="momentum", risk_appetite=0.8,
                           reaction_speed=0, contrarian=False, noise=0.0)
        pos = agent.react(price_change=0.05, sentiment=0, step=0, crowd_position=0)
        assert pos > 0  # should go long on positive shock

    def test_swarm_agent_mean_reversion_fades(self):
        from engine.micro_swarm import SwarmAgent

        agent = SwarmAgent(name="MR1", agent_type="mean_reversion", risk_appetite=0.8,
                           reaction_speed=0, contrarian=True, noise=0.0)
        pos = agent.react(price_change=0.05, sentiment=0, step=0, crowd_position=0)
        assert pos < 0  # should fade the positive move

    def test_swarm_agent_delayed_reaction(self):
        from engine.micro_swarm import SwarmAgent

        agent = SwarmAgent(name="Inst1", agent_type="institutional", risk_appetite=0.5,
                           reaction_speed=5, contrarian=False, noise=0.0)
        pos = agent.react(price_change=0.05, sentiment=0, step=2, crowd_position=0)
        assert pos == 0.0  # not reacting yet

    def test_run_swarm_simulation_bearish_shock(self):
        from engine.micro_swarm import run_swarm_simulation

        np.random.seed(42)
        result = run_swarm_simulation(shock_pct=-8.0, sentiment=-0.8, rsi=72, adx=30,
                                      n_simulations=20, ticker="NVDA")
        assert result["convergence"] in ("BUY", "SELL", "MIXED")
        assert 0 <= result["conviction"] <= 100
        assert result["convergence_speed"] in ("fast", "medium", "slow", "none")
    ...
    def test_run_swarm_simulation_neutral(self):
        ...
        np.random.seed(123)
        result = run_swarm_simulation(shock_pct=0.0, sentiment=0.0, rsi=50, adx=15,
                                      n_simulations=20)
        assert result["buy_pct"] + result["sell_pct"] + result["mixed_pct"] == 100

    def test_run_swarm_simulation_sample_path(self):
        from engine.micro_swarm import run_swarm_simulation

        np.random.seed(7)
        result = run_swarm_simulation(shock_pct=3.0, sentiment=0.5, rsi=45, adx=28,
                                      n_simulations=10)
    ...
    def test_run_swarm_simulation_strong_bullish(self):
        ...
        np.random.seed(0)
        result = run_swarm_simulation(shock_pct=10.0, sentiment=1.0, rsi=35, adx=35,
                                      n_simulations=30)
        # Strong bullish inputs should bias toward BUY
...
class TestPerspectivePanel:
    ...
    def test_build_panel_context(self):
        ...
        summary = {"close": 150.0, "ma20": 145, "ma50": 140,
                    "rsi": 55, "macd_hist": 0.3, "bb_pct": 0.6,
                    "adx": 28, "volume_ratio": 1.2}
    ...
    def test_perspective_result_dataclass(self):
        ...
        pr = PerspectiveResult(
            role="Conservative Analyst", icon="shield",
            bias="BEARISH", score=-30, conviction=65,
            reasoning="Downside risk.", key_factor="Support broken",
        )
    ...
    @patch("engine.role_memory.RoleMemory", side_effect=Exception("no mem"))
    def test_run_perspective_panel_all_bullish(self, mock_mem_cls, mock_logged, mock_client):
        from llm.perspective_panel import run_perspective_panel

        payload = json.dumps({
            "bias": "BULLISH", "score": 50, "conviction": 70,
            "reasoning": "Strong momentum.", "key_factor": "Breakout",
        })
        resp = MagicMock()
        block = MagicMock(text=payload)
        block.type = "text"
        resp.content = [block]
        mock_logged.return_value = (resp, {})

        summary = {"close": 100, "ma20": 95, "ma50": 90, "rsi": 55,
                    "macd_hist": 0.2, "bb_pct": 0.5, "adx": 25, "volume_ratio": 1.1}
    ...
    @patch("llm.perspective_panel.get_client")
    @patch("llm.perspective_panel.logged_create", side_effect=Exception("API error"))
    @patch("engine.role_memory.RoleMemory", side_effect=Exception("no mem"))
    def test_run_perspective_panel_all_fail(self, mock_mem_cls, mock_logged, mock_client):
        from llm.perspective_panel import run_perspective_panel

        summary = {"close": 100, "ma20": 95, "ma50": 90, "rsi": 50,
                    "macd_hist": 0, "bb_pct": 0.5, "adx": 20, "volume_ratio": 1.0}

        result = run_perspective_panel(summary, "NVDA")
        assert result["consensus"] == "NEUTRAL"
        # All failed -> conviction 0 -> no valid perspectives
        assert result["agreement"] == 0

    @patch("llm.perspective_panel.get_client")
    @patch("llm.perspective_panel.logged_create")
    @patch("engine.role_memory.RoleMemory", side_effect=Exception("no mem"))
    def test_run_perspective_panel_mixed(self, mock_mem_cls, mock_logged, mock_client):
        from llm.perspective_panel import run_perspective_panel

        # Return alternating bullish/bearish
        call_count = {"n": 0}
        def side_effect(*args, **kwargs):
            call_count["n"] += 1
            if call_count["n"] % 2 == 1:
                data = {"bias": "BULLISH", "score": 40, "conviction": 60,
                        "reasoning": "up", "key_factor": "momentum"}
            else:
                data = {"bias": "BEARISH", "score": -40, "conviction": 60,
                        "reasoning": "down", "key_factor": "resistance"}
            resp = MagicMock()
            blk = MagicMock(text=json.dumps(data))
            blk.type = "text"
            resp.content = [blk]
            return (resp, {})
        ...
        summary = {"close": 100, "ma20": 100, "ma50": 100, "rsi": 50,
                    "macd_hist": 0, "bb_pct": 0.5, "adx": 20, "volume_ratio": 1.0}

        result = run_perspective_panel(summary, "NVDA")
        assert len(result["perspectives"]) == 4
        assert result["panel_summary"]  # not empty

    def test_call_perspective_invalid_bias_normalized(self):
        from llm.perspective_panel import _call_perspective

        client = MagicMock()
        role = {"name": "Test", "icon": "X", "system": "test", "focus": "test"}
        payload = json.dumps({
            "bias": "INVALID", "score": 10, "conviction": 50,
            "reasoning": "ok", "key_factor": "ok",
        })
        resp = MagicMock()
        block = MagicMock(text=payload)
        block.type = "text"
        ...
        with patch("llm.perspective_panel.logged_create", return_value=(resp, {})):
            result = _call_perspective(client, role, "ctx", "NVDA")
        assert result.bias == "NEUTRAL"  # invalid -> NEUTRAL
    ...
    def test_call_perspective_score_clamped(self):
        ...
        role = {"name": "Test", "icon": "X", "system": "test", "focus": "test"}
        payload = json.dumps({
            "bias": "BULLISH", "score": 999, "conviction": 200,
            "reasoning": "ok", "key_factor": "ok",
        })
        resp = MagicMock()
        block = MagicMock(text=payload)
